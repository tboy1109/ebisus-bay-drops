...
contract Crobots is IDrop, BaseDrop {
    ...
    uint256 private whitelistCost = 225 ether;
    uint256 private memberCost = 200 ether;
    uint256 private regularCost = 250 ether;
    uint64 constant MAX_TOKENS = 2500;
    uint8 constant MAX_MINTAMOUNT = 10;
    uint8 private currentChunkIndex;
    mapping(address => bool) whitelist;
    string baseURI = "ipfs://QmVzPYbXG8meZbFMZBrqpGXyWPBRKepMFVuHsaexhnW77A";
    
    constructor(address _memberships, address _artist) ERC721("Crobots", "CROBOTS") {
    ...
    function mintAirdrop(address[] calldata airdropAddresses) external onlyOwner {
        uint len = airdropAddresses.length;
        for (uint i = 0; i < len; i ++) {
            safeMint(airdropAddresses[i]);
        }
    }
   ...
   function safeMint(address _to) private {
        ...
        // consider we mint 150 nfts for aritst;
        require(tokenId < MAX_TOKENS - 150, "sold out!");
